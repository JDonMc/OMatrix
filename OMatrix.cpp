 ...
 *
 * This file is part of the OMatrix library.
 *
 * OMatrix is free software: you can redistribute it and/or
 * modify it under the terms of the GNU Lesser General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * OMatrix is distributed in the hope that it will be useful,
...
/*!
  ...
  @param   t  Pixel type -- add together NEO_* constants defined in
              OMatrix.h, for example NEO_GRB+NEO_KHZ800 for
              NeoPixels expecting an 800 KHz (vs 400 KHz) data stream
              with color bytes expressed in green, red, blue order per
              pixel.
  @return  OMatrix object. Call the begin() function before use.
*/
OMatrix::OMatrix(uint16_t n, uint16_t p, neoPixelType t) :
...
/*!
  @brief   Deallocate OMatrix object, set data pin back to INPUT.
*/
OMatrix::~OMatrix() {
  free(pixels);
...
/*!
  @brief   Change the length of a previously-declared OMatrix
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           type).
*/
void OMatrix::updateLength(uint16_t n) {
...
/*!
  @brief   Change the pixel format of a previously-declared
           OMatrix strip object. If format changes from one of
  ...
  @note    This function is deprecated, here only for old projects that
           ...
           (length, pin, type).
*/
void OMatrix::updateType(neoPixelType t) {
...
*/
void OMatrix::setPin(uint16_t p) {
...
*/
void OMatrix::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t *p,
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
*/
void OMatrix::fill(uint32_t c, uint16_t first, uint16_t count) {
  uint16_t i, end;
...
*/
uint32_t OMatrix::ColorHSV(uint16_t hue, uint8_t sat, uint8_t val) {
...
*/
uint32_t OMatrix::getPixelColor(uint16_t n) const {
...
*/
void OMatrix::setBrightness(uint8_t b) {
...
*/
uint8_t OMatrix::getBrightness(void) const {
  return brightness - 1;
}
...
*/
void OMatrix::clear(void) {
  memset(pixels, 0, numBytes);
}

// A 32-bit variant of gamma8() that applies the same function
// to all components of a packed RGB or WRGB value.
uint32_t OMatrix::gamma32(uint32_t x) {
  uint8_t *y = (uint8_t *)&x;
